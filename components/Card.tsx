...
const Card: React.FC<CardProps> = ({ job, showBookmarkedOnly = false }) => {
  ...
  React.useEffect(() => {
    ...
    setIsBookmarked(
      existingBookmarks.some((bookmark: Job) => bookmark.id === job.id)
    );
  }, [job.id]);
  const handleBookmark = (e: React.MouseEvent) => {
    ...
    if (isBookmarked) {
      ...
      const updatedBookmarks = existingBookmarks.filter(
        (bookmark: Job) => bookmark.id !== job.id
      );
  ...
  return (
    <div className="w-[21rem] h-48 bg-white shadow-sm rounded-lg border hover:bg-gradient-to-r from-[#fff7e7] to-[#ffffff] hover:cursor-pointer">
